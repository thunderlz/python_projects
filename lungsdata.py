...
# data={'modules' : 'FAutoGlobalStatis,FAutoContinentStatis,FAutoGlobalDailyList,FAutoCountryWeekCompRank,FAutoCountryConfirmAdd,FAutoContinentConfirmStatis,FAutoConfirmMillionRankList'}
...
data = json.loads(re.findall('\{.*\}', bsobj.p.string)[0])
# 获取国际的数据
...
html_global = session.post(url_global, headers=headers)
...
engine.execute('insert into jsondata values(%s,%s)',
               str(data),datetime.now().strftime('%Y%m%d%H%M%S'))
engine.execute('insert into jsondataglobal values(%s,%s)',
